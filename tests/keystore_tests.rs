...
mod test {
    ...
    use rust_bls_bn254::keystores::{base_keystore::Keystore, pbkdf2_keystore::Pbkdf2Keystore, scrypt_keystore::ScryptKeystore};
    use serde_json::json;
    ...
    fn setup() {
        unsafe {
            INIT.call_once(|| {
                ...
                temp_vector.push(serde_json::from_reader(file1).expect("Unable to parse keystore vectors"));

                let file2 = fs::File::open("tests/test-vectors/keystores/keystore-test-1.json")
                    .expect("Unable to open keystore vectors file");
                temp_vector.push(serde_json::from_reader(file2).expect("Unable to parse keystore vectors"));
    ...
    fn test_json_serialization() {
        ...
        for each_file in test_vectors {
            let keystore_json_str = fs::read_to_string(Path::new(&format!("{}/{}/{}/{}", "tests", "test-vectors", "keystores", each_file))).unwrap();
            let expected_json: serde_json::Value = serde_json::from_str(&keystore_json_str).unwrap();
            let expected_json_keystore: Keystore = serde_json::from_str(&keystore_json_str).unwrap();
            let loaded_via_keystore_fn = Keystore::from_file(&format!("{}/{}/{}/{}", "tests", "test-vectors", "keystores", each_file)).unwrap();
    ...
    fn test_encrypt_decrypt_test_vectors() {
        ...
        let binding = hex::decode("000000000019d6689c085ae165831e934ff763ae46a2a6c172b3f1b60a8ce26f").unwrap();
        let test_vector_secret: &[u8] = binding.as_slice();
    
        for tv in test_vector_keystores {
            let iv = tv.crypto.cipher.params.get("iv").unwrap().as_str().unwrap().as_bytes().to_vec();
            let salt = tv.crypto.kdf.params.get("salt").unwrap().as_str().unwrap().as_bytes().to_vec();
            let generated_keystore = if tv.crypto.kdf.function.contains("pbkdf") {
                let mut keystore = Pbkdf2Keystore::new();
                keystore.encrypt(test_vector_secret, TEST_VECTOR_PASSWORD, &tv.path, Some(salt), Some(iv)).unwrap();
    ...
    fn test_generated_keystores() -> Result<(), Box<dyn std::error::Error>> {
        ...
        for tv in test_vector_keystores {
            let aes_iv = tv.crypto.cipher.params.get("iv").and_then(|v| Some(v.as_str().unwrap().as_bytes().to_vec())).unwrap();
            let kdf_salt = tv.crypto.kdf.params.get("salt").and_then(|v| Some(v.as_str().unwrap().as_bytes().to_vec())).unwrap();

            let generated_keystore = if tv.crypto.kdf.function.contains("pbkdf") {
                let mut keystore = Pbkdf2Keystore::new();
                keystore.encrypt(test_vector_secret, TEST_VECTOR_PASSWORD, &tv.path, Some(kdf_salt), Some(aes_iv)).unwrap();
                keystore.to_keystore()
            } else {
                let mut keystore = ScryptKeystore::new();
                keystore.encrypt(test_vector_secret, TEST_VECTOR_PASSWORD, &tv.path, Some(kdf_salt), Some(aes_iv)).unwrap();
    ...
    fn test_encrypt_decrypt_incorrect_password() {
        ...
        let decrypted_secret = keystore.decrypt(String::from_utf8(incorrect_password).unwrap().as_str());
